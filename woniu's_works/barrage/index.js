let data = [
    {value:'周杰伦的听妈妈的话，我反复很久。', time: 5, color: 'red', speed: 1, fontSize: 22},
...
class CanvasBarrage {
    ...
    renderBarrage() {
        ...
        this.barrages.forEach(barrage => {
            ...
            if (!barrage.flag && time >= barrage.time) {
                // 判断当前弹幕是否有过初始化
                // 如果isInit还是false，呢就对当前弹幕进行初始化
                if (!barrage.isInit) {
                    barrage.init()
                    barrage.isInit = true
                }
                // 弹幕要从右往左渲染，所以X的坐标减去当前弹幕的speed即可
    ...
    add(obj) {
        // 实际上就是往barrages数组里面添加一项Barrage的实例而已
